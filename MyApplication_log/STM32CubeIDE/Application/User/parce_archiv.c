...
uint32_t open_read_state_file_archiv(TYPE_STATE_FILE type_state_file)
{
	...
	return data_time;
#else
	return read_state_file_archiv(type_state_file);
#endif
}
		...
		uint8_t ch_type_fil, uint8_t units_fil, uint8_t formula_fil)
{
	...
	while(log_search_by_filter(&data[0], type_log, channel_num_fil, loop_state_fil, thld_state_fil, err_state_fil, ch_type_fil, units_fil, formula_fil) &&
			(search_count_log--))
	{
		// Запись строчки(лога) в файл
		writing_found_logs(&data[0]);
		count_log++;
	}
	*time_start = time_start_log;
...
#else
	return (position + sizeof(TileInfoTypeDef));
#endif
}
